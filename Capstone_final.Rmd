...
```{r types plots 1 & 2, echo = FALSE, warning=FALSE, message=FALSE, fig.cap='Distribution by frequency and averate amount of transactions', fig.width=12}
...
```{r types plot 3, echo = FALSE, warning=FALSE, message=FALSE, fig.cap='Box plot of transactions by amount', fig.width=12}
p3
```
...
Exploring further we can ask the data the following questions: Are there multiple transactions by the same origin flagged as fraud?
Answer: `r subset(flagged, nameOrig %in% paste(not_flagged$nameOrig, not_flagged$nameDest))`. Are there transactions initiated by a destination account flagged as fraud? Answer: `r subset(flagged, nameDest %in% not_flagged$nameOrig)`. Are there multiple transactions where the destination account was flagged? Answer: `r subset(flagged, nameDest %in% not_flagged$nameDest)`.
...
flagged %>% filter(oldbalanceDest == newbalanceDest,
                   oldbalanceOrg == newbalanceOrig)  %>% kable(caption = 'All cases where the isFlaggedFraud is set')
```
...
The package ComplexHeatmap provides great options to create customizeable heatmaps with many options. The figure *Heatmaps of correlations of transactions* display the level of correlation among the features. A correlation exposes the measure of dependence between two quantities, most broadly used is the Pearson correlation. A correlation coefficient of 1 is a perfect direct linear relationship, and -1 represents a perfect inverse correlation. The closer the value is to either -1 or 1 the stronger the correlation between the features. We can observe this on the forementioned heat map.
...
In SVM, a data point is viewed as a *p*-dimensional vector (p is the number of features), the objective is to separate the data points using a (p -1)-dimensional hyperplane, in other words, be able to determine a *new* data point without having the label for the class (our *isFraud* variable).
...
Our SVM model with linear kernel achieved this performance: `r CM_SVM`
...
pred_valid_poly <- predict(model_svm_poly, newdata = svm_validation_set[,-ncol(svm_d)])  # make predictions with validation data
# Confusion matrix
...
# Join X and Y to make a balanced split of the data for validation set. Force all features to numeric type.
d <- cbind(X,isFraud = Y)
d <- d %>% mutate(step = as.numeric(step),
                 type = as.numeric(type),
                 isFraud = as.numeric(isFraud))
...
We will use cross-validation to find the optimal value of the *nrounds* parameter, here we find the settings.
...
The prefered format for the input data is xgb.DMatrix, so we start out by converting our data to this format. Next we determine all the initial parameters and call the xgb.cv() function. With the nrounds defined we are finally able to train the model. After the training we can view the information by typing the name of the model on the console:  `r xgb_model`

```{r xgboost model info, include = FALSE}
...
The algorithm does not use the test_set data during the construction of the model. When making classifications we can do a regression to the *isFraud* label and use a threshold (0.5). We finally calculate the Confusion Matrix to evaluate the performance of the 

### XGBoost results

The final performance results for the XGBoost model can be seen on the table *XGBoost Results*. `r xgb_results`


# Overall Results

No that we have downloaded, decompressed, loaded to the memory, explored, created visualizations, and applied two machine learning models, it is time to see all the results in one place. The table *SVM and XGBoost Result* brings it all together. `r all_results`
...
The motivation behind choosing the PaySim dataset as starting point to this project was *find a real world problem and try to solve it*.  Up to this date Financial Crimes such as Fraud impacts Merchants and Consumers alike, since the first will eventually raise the cost of their products/services to cover financial losses. I am quite pleased with the results achieved here.  As final remarks, it is useful to know that training models on datasets with millions of observations can take a long time and it helps to reduce the size of the data to figure initial parameters and testing how models work can be a time saver.  Specially when dealing with large datasets and working on a single computer with i5 processor and 8GB RAM.  This project has been a great joy and motivation to keep learning R programming and exploring new package and resources.  I look forward to exploring other machine learning challenges with other datasets for deepen my skill set. 
